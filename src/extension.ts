...
export function activate(context: ExtensionContext) {
    let encodedRelativePath = new EncodedRelativePath();

    // The command has been defined in the package.json file
    // Now provide the implementation of the command with  registerCommand
    // The commandId parameter must match the command field in package.json
    let disposable = commands.registerCommand("extension.encodedRelativePath", () => {
        // The code you place here will be executed every time your command is executed

        encodedRelativePath.findRelativePath();
    });

    context.subscriptions.push(encodedRelativePath);
    context.subscriptions.push(disposable);
}

class EncodedRelativePath {
    ...
    constructor() {
        ...
        this._configuration = workspace.getConfiguration("encodedRelativePath");
    ...
    public findRelativePath() {
        ...
        if (disableQuickFilter) {
            const placeHolder = `Found ${this._items.length} files. Enter the filter query. Consider adding more 'encodedRelativePath.ignore' settings.`;
